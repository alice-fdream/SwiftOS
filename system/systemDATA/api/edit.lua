...
local ssh_texts = require("text")
local unicode = require("unicode")
local gpu = require('component').gpu
local themes = require('get_them')
css_ref()

    gpu.setForeground(input_text)
    gpu.setBackground(input_back)
...
local function loadConfig()
  ...
  local env = {}
  local config = loadfile("system/SwiftOS/configs/edit.cfg", nil, env)
  if config then
    pcall(config)
  end
  -- Fill in defaults.
  ...
  if not config then
    ...
    if root and not root.isReadOnly() then
      local f = io.open("system/SwiftOS/configs/edit.cfg", "w")
      if f then
...
local function setStatus(value)
  ...
  value = ssh_texts.padRight(value, w - 10)
...
local function setCursor(nbx, nby)
  ...
  locstring = ssh_texts.padLeft(locstring, 10)
...
local function onClipboard(value)
  ...
  if l then
    repeat
      ...
      next_line = ssh_texts.detab(next_line, 2)
      insert(next_line)
      enter()
      start = l + 1
      l = value:find("\n", start, true)
